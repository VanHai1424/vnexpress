@extends('clients.layouts.master')

@section('content')
    <div class="mt-[28px] mx-auto w-[1100px]">
        <!-- Nav category -->
        <ul class="flex ">
            <li class="py-4 mx-3"><a class="w-full h-full hover:text-[#087cce] font-bold"
                    href="{{ route('category', $category->id) }}">{{ $category->name }}</a></li>
        </ul>
        <!-- Phần content top -->
        <div class="grid grid-cols-[3fr_1fr] ">
            <div class="pr-[20px]">
                <!-- Phần content chính -->
                <div class="">
                    <h2 class="text-[32px] font-bold mb-8">{{ $post->title }}</h2>
                    <span class="block">{{ $post->desc }}</span>
                    <img class="w-full my-8" src="{{ $post->poster }}" alt="">
                    <div class="mb-[20px]">
                        {{ $post->content }}
                    </div>
                </div>
                <!-- Phần bình luận -->
                <div class="">
                    <h2 class="text-[18px] pb-4 ">Ý kiến</h2>
                    <div class="my-2">
                        <form id="form-comment">
                            <textarea placeholder="Chia sẻ ý kiến của bạn"
                                class="border-l-[2px] outline-none p-2 border-[#B42652] rounded-l-[8px] w-full h-[120px] bg-[#F7F7F7]"
                                id="input-comment"></textarea>
                            <div class="flex justify-end mt-2 mb-8">
                                <button type="submit"
                                    class="bg-[#C92A57] hover:bg-[#9F224E] text-white py-1.5 px-6 rounded-md ">Gửi</button>
                            </div>
                        </form>
                    </div>
                    <div class="border-b-[2px] border-[#B42652]"></div>
                    <!--những bình luận -->
                    <div id="block-comment" class="list_comments mt-6">
                        @foreach ($comments as $comment)
                            <div class="flex items-start mb-6">
                                <img class="w-[36px] h-[36px] object-cover mr-[12px]"
                                    src="data:image/png;base64,"
                                    alt="">
                                <div class="">
                                    <span class="font-bold mr-2 ">{{ $comment->user->name }}</span>
                                    <span class="">{{ $comment->content }}</span>
                                </div>
                            </div>
                        @endforeach

                    </div>
                </div>

                <div class="pt-[40px] mb-[20px] border-b-[1.5px]"></div>

                @include('clients.blocks.new_posts')
            </div>
            @include('clients.blocks.sidebar')
        </div>
    </div>
@endsection

@section('script')
    <script>
        $('#form-comment').on('submit', function(e) {
                e.preventDefault();
                $.ajax({
                    url: '{{route("send-comment")}}',
                    method: "POST",
                    data: {
                        _token: '{{csrf_token()}}',
                        post_id: '{{$post->id}}',
                        content: $('#input-comment').val()
                    },
                    success: function(response) {
                        $('#block-comment').html(response.html)
                        $('#input-comment').val('')
                    },
                    error: function(error) {
                        console.log(error);
                    }
                })
            })
    </script>
@endsection
